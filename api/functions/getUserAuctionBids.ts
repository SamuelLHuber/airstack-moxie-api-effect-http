...
export const getUserAuctionBids = (userAddress: string) =>
  ...
  `;
    ...
    const createRequest = Effect.gen(function* (_) {
      ...
      return yield* _(
        HttpClientRequest.jsonBody(requestWithHeaders, { query, variables }),
      );
    });
    ...
    const response = yield* _(
      Effect.retry(HttpClient.fetchOk(request), retryPolicy),
    );
    const result = yield* _(
      HttpClientResponse.schemaBodyJson(ApiResponse)(response),
    );
